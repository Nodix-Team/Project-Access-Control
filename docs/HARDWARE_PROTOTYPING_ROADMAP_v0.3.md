...
* **Option B: ESP32-WROOM-32D (38-Pin NodeMCU-32S / Extended)**

### 🏆 Keputusan Final: Memakai **ESP32-WROOM-32 (30-Pin)**

#### Alasan Teknis & Kompatibilitas Breadboard:
1. **Lebar Board (Ergonomi Breadboard)**:
   * **ESP32-WROOM-32 (30-Pin)** memiliki lebar 25.4mm (1.0 inch). Saat dicolok di selokan tengah breadboard MB-102 standar, ia **menyisakan 1 baris lubang terbuka di sisi kiri DAN sisi kanan**. Kabel jumper Male-to-Male dapat dicolok secara rapi tanpa kabel extension.
   * **ESP32-WROOM-32D (38-Pin)** memiliki lebar 28.5mm (1.12 inch). Saat dicolok, board ini **menutup seluruh lubang di salah satu sisi breadboard**, sehingga memaksa penggunaan 2 breadboard sekaligus atau kabel melayang yang rawan lepas saat diuji.
2. **Kebutuhan Antena RF vs Ethernet**:
   * Seri 32D memiliki keunggulan pada antena RF WiFi. Namun karena arsitektur v0.3 **100% menggunakan Ethernet W5500 via kabel SPI (tanpa WiFi untuk MQTT)**, keunggulan antena 32D sama sekali tidak memberikan dampak pada performa sistem.

---

## 🛡️ 2. Strategi Transisi Seamless ke ESP32-S3 (Hardware Abstraction Layer / HAL)

Untuk menjamin kodingan firmware yang dibuat di breadboard saat ini dapat berpindah ke **ESP32-S3-WROOM-1-N16** (PCB Produksi Final) tanpa merusak atau mengubah logika program, firmware akan menggunakan abstraksi pin via file header `pin_config.h`:
...
#define BOARD_TARGET_PROTOTYPE_WROOM32_30PIN   // <- Aktif untuk Prototyping Lab (ESP32 30-Pin)
// #define BOARD_TARGET_PRODUCTION_ESP32S3_N16 // <- Aktif untuk PCB Produksi (Ref: KEPUTUSAN_ARSITEKTUR_v0.3.md §1.1)

#ifdef BOARD_TARGET_PROTOTYPE_WROOM32_30PIN
  // === MAPPING PIN PROTOTYPING BREADBOARD (ESP32 30-PIN) ===
  ...
  #define PIN_WIEGAND_D1     15  // Disediakan terpisah dari CS Ethernet
...
## 📌 3. Tabel Pinout Fisik ESP32 30-Pin (Breadboard Prototyping)
...
|:---:|:---:|---|:---:|---|
| 24/07/2026 | Initial | Penyusunan Roadmap & Pin Abstraction | **PASS** | Branch `feature/v0.3-hardware-prototyping` dibuat. Pilihan board 30-Pin & HAL `pin_config.h` dibekukan. |
| ... | Fase 1 | ... | ... | ... |
